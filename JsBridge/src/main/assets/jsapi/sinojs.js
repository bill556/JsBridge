(function () {
    ...
    if (zfHelper.isIOS()) {
        if (window.webkit && window.webkit.messageHandlers) {
            __sino = window.webkit.messageHandlers
        }
        if (!__sino) return "inject err: __sino not found"
        delete window.webkit.messageHandlers
    } else {
        __sino = window.__sino
        if (!__sino) return "inject err: __sino not found"
        delete window.__sino
    }
    ...
    if (__DL >= __LogLvl)
        __Log(__DL, "__sino inject VerifyRandomStr:" + _xxyy)

    //防止方法被替换
    var _handleMessageIdentifier = _handleMessageFromNative;
    ...
    function _doSendMessage(msgStr) {
        ...
        if (zfHelper.isIOS()) {
            __sino._sendMessage.postMessage(msgStr)
        } else {
            __sino._sendMessage(msgStr)
        }
    }
     ...
     * Native调用
     */
    function _handleMessageFromNative(msgStr) {
        var curFuncIdentifier = ___sinoJSBridge._handleMessageFromNative
        ...
        if (__DL >= __LogLvl)
            __Log(__DL, '_handleMessageFromNative , msgWrapStr : ' + msgWrapStr)

        if (msgSha !== shaStr) {
            if (__DL >= __LogLvl)
                __Log(__DL, '_handleMessageFromNative shaStr not match, shaStr : ' + shaStr + ' , str : ' + str + ' , msgSha : ' + msgSha)
            return '{}'
        }
        ...
        switch (msgWrap[_MESSAGE_TYPE]) {
            case 'callback': {
                if (typeof msgWrap[_CALLBACK_ID] === 'string' && typeof _callback_map[msgWrap[_CALLBACK_ID]] === 'function') {
                    ...
                    if (__DL >= __LogLvl && hasRet)
                        __Log(__DL, "_handleMessageFromNative , callBackRetStr : " + callBackRetStr)
            ...
            case 'event': {
                if (typeof _event_map[msgWrap[_EVENT_NAME]] === 'function') {
                    ...
                    if (__DL >= __LogLvl && hasRet)
                        __Log(__DL, "_handleMessageFromNative , eventRetStr : " + eventRetStr)
                    return eventRetStr
                }
    ...
    */
    function _call(apiName, params, callback) {
        var curFuncIdentifier = ___sinoJSBridge.call
    ...
    try {
        Object.defineProperty(___sinoJSBridge, '_handleMessageFromNative', {
            value: _handleMessageFromNative,
            writable: false,
            configurable: false
        });
    } catch (e) {
        __Log(__EL, "define _handleMessageFromNative", e)
        return "sinoJSBridge init err:" + e.message
    }

    try {
        Object.defineProperty(window, 'sinoJSBridge', {
            value: ___sinoJSBridge,
            writable: false,
            configurable: false
        })
    } catch (e) {
        __Log(__EL, "define sinoJSBridge", e)

...
static NSString*const imageUrlPreOC  = @"content://sinocare.temp.apps.photos.contentprovider";
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request
{
    ...
    if  ([scheme caseInsensitiveCompare:@"content"]  == NSOrderedSame )
    {
        //看看是否已经处理过了，防止无限循环
...
- (void)startLoading
{
    ...
    //content://sinocare.ios.apps.photos.contentprovider/Documents/A1F74C59-72F0-4470-9456-52DB4EE27A45.png
    NSString *urlString = mutableReqeust.URL.absoluteString;
    
    if ([urlString containsString:imageUrlPreOC] && [urlString hasPrefix:imageUrlPreOC]){
        NSString *lastPath = [urlString substringFromIndex:imageUrlPreOC.length];
        NSString *fullPath = [NSHomeDirectory() stringByAppendingString:lastPath];
